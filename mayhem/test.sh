#!/usr/bin/env bash
#
# mayhem/test.sh — RUN python-bibtexparser's own test suite (deps already installed by
...
# parsed entries / round-tripped BibTeX strings), so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest + atheris + pylatexenc into the in-image site
# dir and compiled the bibtexparser_run_tests ELF wrapper. We only RUN the suite. The suite is
# routed through that compiled NON-system wrapper so the gate's sabotage check (neuter non-system
...
# Put the in-image site dir (atheris + pytest + pylatexenc) and the bibtexparser source tree on
# PYTHONPATH.
...
RUNNER="$SRC/bibtexparser_run_tests"
if [ ! -x "$RUNNER" ]; then
...
# Run the ENTIRE upstream suite under tests/ (unit + middleware + splitter tests; pytest collects
# them natively). -p no:cacheprovider keeps the read-only image happy.
LOG="$(mktemp)"
