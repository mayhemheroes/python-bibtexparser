#!/usr/bin/env bash
#
# mayhem/build.sh — build the python-bibtexparser Atheris fuzz harness + its standalone
...
#      then install atheris + pytest + pylatexenc OFFLINE from that wheelhouse into a fixed
#      site dir on PYTHONPATH. The first (CI, online) build fills the wheelhouse; the
#      air-gapped PATCH re-run resolves entirely from it (pip --no-index --find-links).
#      bibtexparser itself is exercised as its editable source tree (repo root on PYTHONPATH),
#      but needs pylatexenc (its one runtime dep) from the wheelhouse.
#   2. Compile launcher.c -> the ELF Mayhem target `bibtexparser_fuzzer` (Atheris is a Python
#      script; Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `bibtexparser_fuzzer-standalone`.
#   4. Compile the pytest ELF runner wrapper `bibtexparser_run_tests` (so the sabotage oracle bites).
#
...
# wrapper, not the fuzzed Python; Atheris instruments the bibtexparser library itself at import time).
set -euo pipefail
...
#    manylinux wheel for this CPython. pytest runs bibtexparser's own suite; pylatexenc is
#    bibtexparser's sole runtime dependency (pyproject: pylatexenc~=2.10).
PKGS=(atheris pytest "pylatexenc~=2.10")
need_download=0
...
if [ "$need_download" -eq 1 ]; then
  ...
  # pylatexenc 2.10 ships only an sdist — build its wheel NOW (online) so the offline
  # --no-index install below (and the air-gapped re-run) never needs build isolation.
  "$PY" -m pip wheel --no-deps --wheel-dir "$WHEELHOUSE" "pylatexenc~=2.10"
else
...
#    site dir holds atheris+pytest+pylatexenc we SKIP the reinstall. bibtexparser itself stays the
#    editable source tree (repo root on PYTHONPATH) so a PATCH agent's edits under bibtexparser/
#    take effect with no reinstall.
if "$PY" -c "import os,glob,sys; sys.exit(0 if (glob.glob(os.path.join('$SITE','atheris*')) and glob.glob(os.path.join('$SITE','pytest*')) and glob.glob(os.path.join('$SITE','pylatexenc*'))) else 1)"; then
...
# bibtexparser is a top-level package at the repo root, so the repo root itself goes on PYTHONPATH.
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, bibtexparser, pytest; print("imports OK: bibtexparser", bibtexparser.__version__)'
...
#    at run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + bibtexparser.
HARNESS="$SRC/mayhem/fuzz_load.py"
echo ">> compiling bibtexparser_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/bibtexparser_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/bibtexparser_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/bibtexparser_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/bibtexparser_fuzzer" "$SRC/bibtexparser_fuzzer-standalone" "$SRC/bibtexparser_run_tests"
